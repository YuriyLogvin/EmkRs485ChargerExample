...
osThreadId tskTransmit2UartHandle;
/* USER CODE BEGIN PV */
...
int main(void)
{
  ...
  osThreadDef(tskMain, tskMainStart, osPriorityNormal, 0, 128);
  tskMainHandle = osThreadCreate(osThread(tskMain), NULL);

  /* definition and creation of tskTransmit2Uart */
  osThreadDef(tskTransmit2Uart, tskTransmit2UartStart, osPriorityIdle, 0, 128);
  tskTransmit2UartHandle = osThreadCreate(osThread(tskTransmit2Uart), NULL);
